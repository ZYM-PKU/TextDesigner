...
class GeneralConditioner(nn.Module):
    ...
    def possibly_get_ucg_val(self, embedder: AbstractEmbModel, batch: Dict) -> Dict:
        ...
        p = embedder.ucg_rate
        assert len(embedder.input_keys) <= 2
        if len(embedder.input_keys) == 1:
            input_key = embedder.input_key
            for i in range(len(batch[input_key])):
                flag = embedder.ucg_prng.choice(2, p=[1 - p, p])
                if flag == 1:
                    batch[input_key][i] = torch.zeros_like(batch[input_key][i])
        else:
            input_key_1, input_key_2 = embedder.input_keys
            assert len(batch[input_key_1]) == len(batch[input_key_2])
            for i in range(len(batch[input_key_1])):
                flag = embedder.ucg_prng.choice(4, p=[1 - 3*p, p, p, p])
                if flag == 1:
                    batch[input_key_1][i] = torch.zeros_like(batch[input_key_1][i])
                elif flag == 2:
                    batch[input_key_2][i] = torch.zeros_like(batch[input_key_2][i])
                elif flag == 3:
                    batch[input_key_1][i] = torch.zeros_like(batch[input_key_1][i])
                    batch[input_key_2][i] = torch.zeros_like(batch[input_key_2][i])
    ...
    def get_unconditional_conditioning(self, batch: Dict) -> Dict:
        ...
        batch_uc[self.ucg_keys[0]] = torch.zeros_like(batch_uc[self.ucg_keys[0]])
        batch_uc["s_ref"] = torch.zeros_like(batch_uc["s_ref"])

        if len(self.ucg_keys) == 2:
            batch_uc_1 = copy.deepcopy(batch)
            batch_uc[self.ucg_keys[1]] = torch.zeros_like(batch_uc[self.ucg_keys[1]])
            batch_uc_1[self.ucg_keys[1]] = torch.zeros_like(batch_uc_1[self.ucg_keys[1]])
...
class FSFReferenceEmbedder(AbstractEmbModel):
    def __init__(
        ...
        assert fuse_mode in ["none", "adain", "fsfont"]
    ...
    def forward_c(self, c_ref):

        b, l, h, w = c_ref.shape
        c_ref = c_ref.reshape(-1, 1, 64, 64).tile(1,self.in_channel,1,1) # (b, l, h, w) -> (bl, in_channel, h, w)
        content_feat = self.content_encoder(c_ref) # (bl, c, h/2, w/2)
        content_feat = self.to_out(content_feat) # (bl, c/2, h/4, w/4)
        content_feat = content_feat.reshape(b, l, -1) # 8192

        return content_feat

    def forward_adain(self, c_ref, s_ref):

        b, l, h, w = c_ref.shape
        c_ref = c_ref.reshape(-1,1,64,64).tile(1,self.in_channel,1,1) # (b, l, h, w) -> (bl, 3, h, w)
        content_feat = self.content_encoder(c_ref) # (bl, c, h/2, w/2)
        
        s_ref = s_ref.reshape(-1,3,64,64) # (b, l, 3, h, w) -> (bl, 3, h, w)
        style_feat = self.style_encoder(s_ref) # (bl, c, h/8, w/8)

        # AdaIN
        style_feat = rearrange(style_feat, 'b c h w -> b (h w) c').contiguous()
        content_feat = rearrange(content_feat, 'b c h w -> b (h w) c').contiguous()

        content_mean = content_feat.mean(dim=-2, keepdims=True) # (bl, 1, c)
        content_std = content_feat.std(dim=-2, keepdims=True) # (bl, 1, c)

        style_mean = style_feat.mean(dim=-2, keepdims=True) # (bl, 1, c)
        style_std = style_feat.std(dim=-2, keepdims=True) # (bl, 1, c)

        out = (content_feat - content_mean) / content_std
        out = out * style_std + style_mean # (bl, hw/4, c)

        out = out.permute(0,2,1).contiguous() # (bl, c, hw/4)
        out = out.reshape(b*l, -1, h//2, w//2) # (bl, c, h/2, w/2)
        out = self.to_out(out) # (bl, c, h/4, w/4)
        out = out.reshape(b, l, -1) # 8192

        return out

    def forward_fsfont(self, c_ref, s_ref):
    ...
    def forward(self, c_ref, s_ref=None):

        if self.fuse_mode == "none":
            return self.forward_c(c_ref)
        elif self.fuse_mode == "adain":
            return self.forward_adain(c_ref, s_ref)
        elif self.fuse_mode == "fsfont":
            return self.forward_fsfont(c_ref, s_ref)
...
class FSFReferenceEmbedder2(AbstractEmbModel):
    def __init__(
        ...
        self.to_out = nn.Sequential(
            nn.GroupNorm(num_groups=self.emb_dim//4, num_channels=self.emb_dim*2),
            nn.SiLU(),
    ...
    def forward(self, c_ref, s_ref, ref_len):
        ...
        for i in range(b):
            ...
            s_ref_rand = choices(list(s_ref_i), k=n*self.n_ref_syle)
            s_ref_rand = torch.cat(s_ref_rand, dim=0).reshape(n, self.n_ref_syle, 3, h, w) # (n, nrs, 3, h, w)
            if l > n:
                s_ref_pad = torch.zeros(l-n, self.n_ref_syle, 3, h, w).to(s_ref_rand)
        ...
        s_ref_b = s_ref.reshape(b*l, self.n_ref_syle, 3, h, w) # (bl, nrs, 3, h, w)

        out = self.model.encode(c_ref_b, s_ref_b) # (bl, 2c, h/4, w/4)
        out = self.to_out(out) # (bl, c, h/16, w/16)
        out = out.reshape(b, l, -1) # (b, l, 2048)
